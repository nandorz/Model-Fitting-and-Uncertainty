...
model1samples.prior3 <-stan("sir_prior3.stan",
                    iter=niter,
                    data=data_sir,
                    chains=3,
                    seed=123, 
...
summary(model1samples.prior3)


model1samples.prior3

model_pred_SIR.prior3 <- model1samples.prior3

pars=c('beta', 'gamma', "R0", "recovery_time")
print(model_pred_SIR.prior3, pars = pars)

#Checking stan outputs
beta_pred<-rstan::extract(model_pred_SIR.prior3)$beta
nu_pred<-rstan::extract(model_pred_SIR.prior3)$gamma
R0_pred<-rstan::extract(model_pred_SIR.prior3)$R0
inv_beta_pred<-rstan::extract(model_pred_SIR.prior3)$recovery_time

# prior predictive check ####
s_prior <- rstan::extract(model_pred_SIR.prior3)
...
smr_pred <- cbind(as.data.frame(summary(model_pred_SIR.prior3, pars = "pred_cases",
...
stan_model <- stan_model(model_code = "model3.stan")

# fit the model to your data
fit <- sampling(stan_model, data=data_sir)

# generate posterior predictive samples
ppc <- posterior_predict(fit)

# calculate the posterior mean deviance
lp <- posterior_linpred(fit)
deviance <- -2 * sum(dnorm(y, lp, sd = 1, log = TRUE))
post_mean_deviance <- mean(deviance)

# calculate the effective number of parameters
pD <- effective_parameters(fit)$pD

# calculate the DIC
DIC <- pD + post_mean_deviance


gammaSamples <- sampling(model1samples, list(y=cases, N=length(cases)))

loo::loo(model1samples, pointwise=T)$pointwise
